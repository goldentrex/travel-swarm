...
const PREFERENCE_STRINGS: Record<SupportedLanguage, LocalizedPreferenceStrings> = {
  en: {
    ...
    dropActivityLabel: "Keep {y}",
    dropActivityDetail: "{x} is dropped to make room",
  ...
  de: {
    ...
    dropActivityLabel: "{y} behalten",
    dropActivityDetail: "{x} entfällt dafür",
  ...
  es: {
    ...
    dropActivityLabel: "Mantener {y}",
    dropActivityDetail: "{x} se cancela a cambio",
  ...
  fr: {
    ...
    dropActivityLabel: "Garder {y}",
    dropActivityDetail: "{x} est abandonné en échange",
  ...
  zh: {
    ...
    dropActivityLabel: "保留{y}",
    dropActivityDetail: "改为放弃{x}",
...
function buildActivityPriorityQuestion(
  ...
  return {
    ...
    options: [
      ...
      {
        // Still `drop_<the first activity>` on the wire — the option id is the
        // frozen contract the constraint derivation reads. Only the WORDS
        // change: "Keep A / Drop A" asked the traveller to choose between one
        // activity and itself, when the real choice is between two.
        id: `drop_${keepSlug}`,
        label: capText(fillTemplate(p.dropActivityLabel, { y: other }), PREFERENCE_LABEL_MAX),
        detail: capText(fillTemplate(p.dropActivityDetail, { x: keep }), PREFERENCE_DETAIL_MAX),
      },
    ],
  };
}
